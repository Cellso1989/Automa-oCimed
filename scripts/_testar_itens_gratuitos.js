...
async function main() {
  ...
  try {
    ...
    // Reaproveita um pedido existente recente (ja aberto/sem itens)
    ...
    console.log("Usando pedido:", pedidoId);
    ...
    let totalRows = await page.getByRole("row").count();
    console.log("Rows ANTES de trocar o modelo:", totalRows);

    // Acha o combobox "Modelo do item do pedido"
    const comboModelo = page.getByRole("combobox", { name: /Modelo do item do pedido/i });
    console.log("Combo 'Modelo do item do pedido' encontrado:", await comboModelo.count());
    await comboModelo.click();
    await page.waitForTimeout(1000);
    await page.getByRole("option", { name: "Itens gratuitos", exact: true }).click();
    await page.waitForTimeout(3000);

    totalRows = await page.getByRole("row").count();
    console.log("Rows DEPOIS de trocar pra 'Itens gratuitos':", totalRows);
    if (totalRows > 1) {
      const texto = await page.getByRole("row").nth(1).innerText({timeout: 5000}).catch(e => "erro: " + e.message);
      console.log("row 1:", texto.replace(/\n/g," | ").slice(0,200));
    }
  } finally {
